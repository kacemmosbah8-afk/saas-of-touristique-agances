...
export function ProviderDetailTabs({
  ...
  const [environment, setEnvironment] = useState(connection?.environment ?? "SANDBOX");
  const [authType, setAuthType] = useState(connection?.authType ?? meta.authType);
  const [baseUrl, setBaseUrl] = useState(connection?.baseUrl ?? "");
  const [autoReconnect, setAutoReconnect] = useState(connection?.autoReconnect ?? true);

  // Credential entry state.
  const [credType, setCredType] = useState<ProviderCredentialType>(meta.credentialTypes[0]);
  ...
  function saveConnection() {
    startTransition(async () => {
      const result = await updateProviderConnectionAction(tenantId, provider.id, {
        environment,
        authType,
        baseUrl,
        autoReconnect,
  ...
  function removeCredential(credentialId: string) {
    startTransition(async () => {
      const result = await deleteProviderCredentialAction(tenantId, credentialId);
      if (!result.ok) {
        toast.error(result.error);
        return;
      }
  ...
  function toggleCapability(capabilityId: string, enabled: boolean) {
    startTransition(async () => {
      const result = await toggleProviderCapabilityAction(tenantId, capabilityId, enabled);
      if (!result.ok) {
        toast.error(result.error);
        return;
      }
  ...
  function removeProvider() {
    if (!confirm(`Remove ${meta.name}? Credentials and logs are kept for audit.`)) return;
    startTransition(async () => {
      ...
      <TabsContent value="connection" className="space-y-8">
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-medium">Environment &amp; Authentication</h3>
            {canEdit && (
              <div className="flex gap-2">
                <Button size="sm" variant="outline" disabled={isPending} onClick={testConnection}>
                  <PlugZap className="mr-1.5 size-4" />
                  Test Connection
                </Button>
                <Button size="sm" disabled={isPending} onClick={saveConnection}>
                  Save
                </Button>
              </div>
            )}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => {
                  ...
                  setBaseUrl(v === "PRODUCTION" ? meta.productionUrl : meta.sandboxUrl);
                }}
                disabled={!canEdit}
              >
            ...
            <div className="space-y-1.5">
              ...
              >
                ...
                <SelectContent>
                  {(["OAUTH", "API_KEY", "SECRET", "CERTIFICATE"] as const).map((t) => (
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Input
                ...
                disabled={!canEdit}
                placeholder={environment === "PRODUCTION" ? meta.productionUrl : meta.sandboxUrl}
              />
            </div>
        ...
        <section className="space-y-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Stored encrypted (AES-256-GCM). Values are never displayed after saving.
            </p>
          </div>
          ...
          {provider.credentials.length > 0 && (
            <ul className="divide-y rounded-lg border">
              {provider.credentials.map((cred) => (
                <li key={cred.id} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                  <KeyRound className="text-muted-foreground size-4 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <p className="font-medium">{CREDENTIAL_TYPE_LABELS[cred.type]}</p>
                    <p className="text-muted-foreground font-mono text-xs">{cred.maskedValue}</p>
                  </div>
                  {cred.expiresAt && (
          ...
          {canManage && (
            <div className="flex flex-wrap items-end gap-2">
              <div className="space-y-1.5">
                ...
                <Select value={credType} onValueChange={(v) => setCredType(v as ProviderCredentialType)}>
              ...
              <Button size="sm" disabled={isPending || !credValue.trim()} onClick={saveCredential}>
                Save Credential
              </Button>
            </div>
          )}
      ...
      <TabsContent value="capabilities">
        <ul className="divide-y rounded-lg border">
          {provider.capabilities.map((cap) => (
            <li key={cap.id} className="flex items-center justify-between px-4 py-3 text-sm">
              <span className="font-medium capitalize">{cap.name.replace(/_/g, " ")}</span>
      ...
      <TabsContent value="webhooks" className="space-y-4">
        {canManage && (
          <div className="flex flex-wrap items-end gap-2">
            ...
            <Button
              size="sm"
              disabled={isPending || !webhookEvent.trim() || !webhookUrl.trim()}
              onClick={addWebhook}
            >
              Add Webhook
            </Button>
        ...
        {provider.webhooks.length === 0 ? (
          ...
          <ul className="divide-y rounded-lg border">
            {provider.webhooks.map((hook) => (
              <li key={hook.id} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                <div className="min-w-0 flex-1">
                  <p className="font-medium">{hook.event}</p>
                  <p className="text-muted-foreground truncate text-xs">{hook.url}</p>
                </div>
                {canManage && (
                  <Button
                    size="icon"
          ...
          <ul className="divide-y rounded-lg border font-mono text-xs">
            {provider.logs.map((log) => (
              <li key={log.id} className="flex items-start gap-3 px-4 py-2">
                ...
                <span className={`w-12 shrink-0 font-semibold ${LOG_LEVEL_CLASS[log.level]}`}>
                  {log.level}
                </span>
          ...
          {provider.syncs.length === 0 ? (
            ...
            <ul className="divide-y rounded-lg border text-sm">
              {provider.syncs.map((sync) => (
                <li key={sync.id} className="flex items-center gap-3 px-4 py-2.5">
                  <span
          ...
          {provider.errors.length === 0 ? (
            ...
            <ul className="divide-y rounded-lg border text-sm">
              {provider.errors.map((error) => (
                <li key={error.id} className="flex items-center gap-3 px-4 py-2.5">
                  <div className="min-w-0 flex-1">
                    <p className={error.resolved ? "text-muted-foreground line-through" : ""}>

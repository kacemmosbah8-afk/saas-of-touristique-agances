...
import { listPublicActivities } from "@/features/public-site/lib/public-cache";
...
export async function generateMetadata({
  ...
  return { title: `${dict.listing.activities.title} — ${tenant.name}` };
}
...
export default async function PublicActivitiesPage({
  ...
  const result = await listPublicActivities(tenant.id, {
    status: "ACTIVE",
    search: q || undefined,
    ...
    <div className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
      <div className="mb-10 flex flex-wrap items-end justify-between gap-6 sm:mb-14">
        ...
        >
          ...
          <input
            ...
            name="q"
            defaultValue={q}
            placeholder={dict.listing.activities.searchPlaceholder}
      ...
      {result.activities.length === 0 ? (
        <p className="text-muted-foreground rounded-lg border border-dashed p-16 text-center">
          {q ? dict.listing.activities.emptySearch : dict.listing.activities.emptyDefault}
        </p>
      ) : (
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5">
                    ...
                    {activity.sellingPrice != null && (
                      <p className="mt-1.5 text-xs font-medium text-white/60">
                        {dict.product.from} {activity.currency} {activity.sellingPrice.toLocaleString()}{" "}
